...
const BRIDGE: &str = r#"(function(){
  ...
  // Persistent design-comment dots for the current page. list=[{id,x,y,text}]; on
  // toggles visibility. Clicking a dot toggles a fixed-width bubble with the text
  // (appended to the wide layer, not the 16px dot, so its width isn't collapsed).
  B.marks = function(list, on){ var layer=document.getElementById('__readoMarks'); if(!layer){ layer=document.createElement('div'); layer.id='__readoMarks'; (document.body||document.documentElement).appendChild(layer); } layer.innerHTML=''; layer.style.display=on?'block':'none'; (list||[]).forEach(function(m){ var d=document.createElement('div'); d.title=m.text; d.style.cssText='position:absolute;z-index:2147483640;left:'+m.x+'px;top:'+m.y+'px;transform:translate(-50%,-50%);width:16px;height:16px;border-radius:50%;background:#ff2d55;border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.4);cursor:pointer'; var b=null; d.onmousedown=function(ev){ ev.stopPropagation(); ev.preventDefault(); if(b){ b.remove(); b=null; return; } b=document.createElement('div'); b.innerHTML=B.esc(m.text); b.style.cssText='position:absolute;z-index:2147483641;left:'+(m.x+14)+'px;top:'+(m.y-6)+'px;width:260px;box-sizing:border-box;background:#20242e;color:#e6e9ef;padding:8px 11px;border-radius:9px 9px 9px 0;box-shadow:0 8px 24px rgba(0,0,0,.45);font:13px -apple-system,BlinkMacSystemFont,sans-serif;white-space:pre-wrap;word-break:break-word;line-height:1.4'; layer.appendChild(b); }; layer.appendChild(d); }); };
