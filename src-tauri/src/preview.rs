...
const BRIDGE: &str = r#"(function(){
  ...
  // Persistent design-comment dots for the current page. list=[{x,y,text}]; on toggles visibility.
  B.marks = function(list, on){ var layer=document.getElementById('__readoMarks'); if(!layer){ layer=document.createElement('div'); layer.id='__readoMarks'; (document.body||document.documentElement).appendChild(layer); } layer.innerHTML=''; layer.style.display=on?'block':'none'; (list||[]).forEach(function(m){ var d=document.createElement('div'); d.style.cssText='position:absolute;z-index:2147483640;left:'+m.x+'px;top:'+m.y+'px;transform:translate(-50%,-50%);width:16px;height:16px;border-radius:50%;background:#ff2d55;border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.4);cursor:pointer'; var open=false,b=null; d.onmousedown=function(ev){ ev.stopPropagation(); ev.preventDefault(); if(open){ if(b)b.remove(); b=null; open=false; return; } b=document.createElement('div'); b.innerHTML=B.esc(m.text); b.style.cssText='position:absolute;left:14px;top:-2px;max-width:240px;background:#111;color:#fff;padding:6px 9px;border-radius:8px;font:12px -apple-system,sans-serif;white-space:pre-wrap;word-break:break-word;line-height:1.35;z-index:2147483641'; d.appendChild(b); open=true; }; layer.appendChild(d); }); };
