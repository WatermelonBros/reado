...
const BRIDGE: &str = r#"(function(){
  ...
  B.drain = function(){ var l=B.logs, ip=B.inspectPath, ca=B.commentAt, oc=B.openComment, cr=B.commentReply, cs=B.commentResolve; B.logs=[]; B.inspectPath=null; B.commentAt=null; B.openComment=null; B.commentReply=null; B.commentResolve=null; return {logs:l, net:B.net.slice(-300), inspect:ip, commentAt:ca||null, openComment:oc||null, commentReply:cr||null, commentResolve:cs||null}; };
  ...
  B.showComment = function(c){ B.closeComment(); var TC={bug:'#ff5c5c',refactor:'#c084fc',performance:'#f59e0b',question:'#38bdf8',note:'#94a3b8'}; var back=document.createElement('div'); back.id='__readoCommentBack'; back.style.cssText='position:fixed;inset:0;z-index:2147483644;background:transparent'; back.onmousedown=function(e){ if(e.target===back) B.closeComment(); }; var card=document.createElement('div'); card.style.cssText='position:absolute;left:'+(c.x+14)+'px;top:'+c.y+'px;width:320px;box-sizing:border-box;background:#1b1f27;color:#e6e9ef;border:1px solid #3a4150;border-radius:12px;box-shadow:0 16px 44px rgba(0,0,0,.55);font:13px -apple-system,BlinkMacSystemFont,sans-serif;overflow:hidden'; var head='<div style="display:flex;align-items:center;gap:8px;padding:9px 12px;border-bottom:1px solid #2b313c"><span style="width:9px;height:9px;border-radius:50%;background:'+(TC[c.type]||'#94a3b8')+'"></span><span style="font-weight:600;text-transform:capitalize">'+B.esc(c.type)+'</span>'+(c.resolved?'<span style="color:#4ade80;font-size:11px">done</span>':'')+'<span style="flex:1"></span><span id="__rcClose" style="cursor:pointer;color:#9aa3b2;padding:0 4px">x</span></div>'; var msgs='<div style="max-height:220px;overflow:auto;padding:10px 12px">'; (c.messages||[]).forEach(function(m){ msgs+='<div style="margin-bottom:10px"><div style="font-size:11px;color:#9aa3b2;margin-bottom:2px"><b style="color:#cbd3e1">'+B.esc(m.who)+'</b> &middot; '+B.esc(m.when)+'</div><div style="white-space:pre-wrap;word-break:break-word;line-height:1.4">'+B.esc(m.body)+'</div></div>'; }); msgs+='</div>'; var foot='<div style="padding:8px 12px;border-top:1px solid #2b313c;display:flex;gap:6px"><input id="__rcReply" placeholder="Reply..." style="flex:1;min-width:0;background:#12151b;color:#e6e9ef;border:1px solid #3a4150;border-radius:6px;padding:6px 8px;font:13px -apple-system;outline:none"><button id="__rcSend" style="border:0;border-radius:6px;background:#3b82f6;color:#fff;padding:0 12px;cursor:pointer">Send</button>'+(c.resolved?'':'<button id="__rcResolve" style="border:0;border-radius:6px;background:#22331f;color:#4ade80;padding:0 10px;cursor:pointer">Resolve</button>')+'</div>'; card.innerHTML=head+msgs+foot; back.appendChild(card); (document.body||document.documentElement).appendChild(back); document.getElementById('__rcClose').onmousedown=function(e){ e.preventDefault(); B.closeComment(); }; var inp=document.getElementById('__rcReply'); function doSend(){ var v=inp.value.trim(); if(v){ B.commentReply={id:c.id, text:v}; inp.value=''; } } document.getElementById('__rcSend').onmousedown=function(e){ e.preventDefault(); doSend(); }; inp.addEventListener('keydown', function(e){ if(e.key==='Enter'){ e.preventDefault(); doSend(); } if(e.key==='Escape'){ B.closeComment(); } }); var rz=document.getElementById('__rcResolve'); if(rz) rz.onmousedown=function(e){ e.preventDefault(); B.commentResolve=c.id; }; setTimeout(function(){ inp.focus(); },0); };
  ...
  function showMenu(x, y, path, px, py){
    ...
    var items=[
      ...
      ['Comment here', function(){ B.compose(px, py); }]
    ];
