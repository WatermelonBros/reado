...
const BRIDGE: &str = r#"(function(){
  ...
  B.drain = function(){ var l=B.logs, ip=B.inspectPath, ca=B.commentAt, oc=B.openComment; B.logs=[]; B.inspectPath=null; B.commentAt=null; B.openComment=null; return {logs:l, net:B.net.slice(-300), inspect:ip, commentAt:ca||null, openComment:oc||null}; };
  ...
  // toggles visibility. The native tooltip previews the text; clicking buffers the
  // id so Reado opens the real comment thread in its sidebar (reusing that UI).
