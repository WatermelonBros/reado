/**
 * App startup, callable.
 *
 * `main.tsx` calls this and nothing else. A build that embeds Reado calls it too,
 * after registering what it adds to the interface (see `lib/slots.ts`) — so the
 * contributions are there on the very first render and the order of everything
 * below is the same whichever entry started it.
 */
...
export function boot() {
  ...
  // Dev-only: placeholders in every UI slot, to check where they sit (see
  // `lib/slotPreview.tsx`). Registered before the first render, like real
  // contributions.
  if (import.meta.env.DEV && import.meta.env.VITE_PREVIEW_SLOTS)
    void import("./lib/slotPreview").then((m) => {
      m.previewSlots()
      render()
    })
  else render()
