...
export function BookmarksPanel() {
  ...
  if (bookmarks.length === 0) {
    return <p className="px-4 py-6 text-xs leading-relaxed text-faint">{t("bookmarks.empty")}</p>
  }
  ...
  return (
    <ul className="m-0 h-full list-none overflow-y-auto p-0 py-1">
      {groups.map((g) => (
        <li key={g.path}>
          <div className="truncate px-3 pt-2 pb-0.5 text-xs font-medium text-muted">{g.path}</div>
          {g.items.map((b) => (
