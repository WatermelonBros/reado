...
describe("GitPanel", () => {
  ...
  it("commits only when a message is typed and something is staged", async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText("git.commitPlaceholder"), "fix things")
  ...
  it("syncs (pull+push) and reports conflicts left to resolve", async () => {
    ...
    await waitFor(() =>
      expect(useNotice.getState().notices.some((n) => n.text.includes("git.syncConflicts"))).toBe(
        true,
      ),
    )
  })
