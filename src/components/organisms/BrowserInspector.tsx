...
export function BrowserInspector({ docked = false }: { docked?: boolean } = {}) {
  ...
  const actions: Act[] = [
    ...
    ...(tab === "elements"
      ? [
          {
            key: "pick",
            label: t("inspector.pick"),
            icon: <CrosshairIcon className="h-3.5 w-3.5" />,
            onClick: () => setPick(!pickMode),
            active: pickMode,
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            onClick: () => void (tab === "application" ? loadStorage() : loadDom()),
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            onClick: attachInspector,
          },
        ]
      : [
          {
            key: "dock",
            label: t("inspector.dock"),
            icon: (
              <LayoutIcon
                className={`h-3.5 w-3.5 ${inspectorPos === "right" ? "rotate-90" : ""}`}
              />
            ),
            onClick: () => setInspectorPos(inspectorPos === "bottom" ? "right" : "bottom"),
          },
          {
            key: "detach",
            label: t("inspector.detach"),
            icon: <DetachIcon className="h-3.5 w-3.5" />,
            onClick: detachInspector,
          },
        ]),
  ]
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden bg-canvas">
      <header
        ref={headerRef}
        className="flex h-8 flex-none items-center gap-1 border-b border-line px-1.5 text-xs"
      >
        <div className="flex min-w-0 flex-none items-center gap-1 overflow-x-auto">
          ...
          <TabButton active={tab === "network"} onClick={() => setTab("network")}>
            {t("inspector.network")}{" "}
            {net.length > 0 && <span className="text-faint">({net.length})</span>}
          </TabButton>
        ...
        {tab === "console" ? (
            ...
            shownLogs.map((l, i) => (
              ...
              >
                <span className="min-w-0 flex-1">
                  ...
                  {l.stack && (
                    <pre className="mt-0.5 whitespace-pre-wrap break-words text-[10px] text-faint">
                      {l.stack}
                    </pre>
                  )}
                </span>
          ...
          shownNet.length === 0 ? (
            ...
            <div className="flex h-full">
              <div
                className={`min-w-0 overflow-auto ${selectedNet != null ? "w-1/2 border-r border-line" : "flex-1"}`}
              >
                {shownNet.map((n) => (
                  <button
                    key={n.id}
                  ...
                  >
                    ...
                    <span className="w-10 flex-none tabular-nums">
                      {n.status ?? (n.error ? "ERR" : "…")}
                    </span>
                    <span className="min-w-0 flex-1 truncate">{n.url}</span>
                    {n.method === "WS" && (n.frames ?? 0) > 0 && (
                      <span className="flex-none text-faint">{n.frames}▾</span>
                    )}
                    {n.ms != null && (
                      <span className="flex-none text-faint tabular-nums">{n.ms}ms</span>
                    )}
                  </button>
                ))}
              </div>
    ...
    h && Object.keys(h).length ? (
      Object.entries(h).map(([k, v]) => (
        <div key={k} className="flex gap-2 px-2 py-0.5">
          <span className="w-40 flex-none truncate text-accent" title={k}>
            {k}
          </span>
  ...
  return (
    <div className="min-w-0 flex-1 overflow-auto">
      <div className="sticky top-0 flex items-center justify-between gap-2 border-b border-line bg-canvas px-2 py-1">
        <span className="min-w-0 truncate text-ink" title={entry.url}>
          {entry.url}
        </span>
        <IconButton
          size="sm"
...
function Pre({ children }: { children: React.ReactNode }) {
  return (
    <pre className="m-0 max-h-40 overflow-auto whitespace-pre-wrap break-words px-2 py-1 text-ink">
      {children}
    </pre>
  )
}
      ...
      {sections.map(([title, kind, rows]) => (
        <section key={kind} className="mb-2">
          ...
          {rows.map(([k, v]) => (
            <div
              key={k}
              className="group/row flex items-center gap-2 border-b border-line/40 px-1 py-0.5"
            >
              <span className="w-40 flex-none truncate text-accent" title={k}>
                {k}
              </span>
