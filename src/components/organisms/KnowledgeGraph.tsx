...
export function KnowledgeGraph() {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    const seed = (i: number, n: number) => ({
      x:
        size.w / 2 +
        Math.cos((i / Math.max(n, 1)) * Math.PI * 2) * 260 +
        (Math.random() - 0.5) * 60,
      y:
        size.h / 2 +
        Math.sin((i / Math.max(n, 1)) * Math.PI * 2) * 260 +
        (Math.random() - 0.5) * 60,
    })
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        {nodes.length > 0 && (
          <footer className="flex flex-none items-center gap-4 border-t border-line px-4 py-2 text-xs text-faint">
