...
import { onPtyOutput, ptyWrite } from "./api"
import { findPanel, useLayout } from "./layout"
import { createLogger } from "./logger"
import { frameBytes, framePtyLines, shellQuote } from "./ptyText"
...
export async function listenPtyLines(
  ...
  return onPtyOutput(id, (chunk) => {
    const bytes = frameBytes(chunk)
    const text = bytes ? decoder.decode(bytes, { stream: true }) : String(chunk)
