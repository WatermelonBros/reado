...
describe("useLayout store", () => {
  ...
  it("stacks without minting a new group when not splitting", () => {
    ...
    // Output and Problems ship in that group too — see `defaultLayout`.
    expect(l.areas.bottom.groups[0].tabs).toEqual(["terminal", "output", "problems", "browser"])
  })
  ...
  it("reset restores the default arrangement and bumps the counter", () => {
    ...
    expect(findPanel(useLayout.getState().layout, "browser")).toEqual({
      area: "right",
      groupId: "g-browser",
    })
