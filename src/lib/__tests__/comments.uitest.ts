...
describe("create / patch / reply / remove", () => {
  ...
  it("reply distributes the updated thread", async () => {
    ...
    api.addReply.mockResolvedValue(
...
describe("re-anchor flow", () => {
  ...
  it("applyReanchor re-binds the comment and clears reanchoring", async () => {
    ...
    api.setAnchor.mockResolvedValue(
      mkComment({ id: "c1", anchor: { file: "b.ts", scope: "range", startLine: 5, endLine: 6 } }),
    )
...
describe("misc", () => {
  ...
  it("setGitignorePrompt / setActive / replaceForFile", () => {
    ...
    useComments.setState({
      comments: [
        mkComment({ id: "x", anchor: { file: "a.ts", scope: "range", startLine: 1, endLine: 1 } }),
      ],
    })
    C().replaceForFile("a.ts", [
      mkComment({ id: "y", anchor: { file: "a.ts", scope: "range", startLine: 9, endLine: 9 } }),
    ])
