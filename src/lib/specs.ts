...
export function groupSpecs(files: string[]): SpecGroup[] {
  const md = files.map((f) => f.replace(/\\/g, "/")).filter((f) => /\.(md|markdown)$/i.test(f))
  ...
  // First pattern that matches wins, so the order below is the precedence:
  // the same file can look like both a change delta and a capability spec.
  for (const path of md) {
    // --- OpenSpec ---
    // A capability spec delta nested under a change → label by capability.
    const specDelta = path.match(/(?:^|\/)\.?openspec\/changes\/([^/]+)\/specs\/([^/]+)\/(.+)$/)
    if (specDelta) {
      const label = specDelta[3] === "spec.md" ? specDelta[2] : `${specDelta[2]}/${specDelta[3]}`
      const name = specDelta[1]
      add(`change:${name}`, { title: name, kind: "change" }, { label, path, isSpec: true })
      continue
    }
    // A change document (proposal/design/tasks) directly under the change.
    const changeDoc = path.match(/(?:^|\/)\.?openspec\/changes\/([^/]+)\/([^/]+)$/)
    if (changeDoc) {
      const name = changeDoc[1]
      add(
        `change:${name}`,
        { title: name, kind: "change" },
        { label: changeDoc[2], path, isSpec: false },
      )
      continue
    }
    // A standalone OpenSpec capability spec.
    const capability = path.match(/(?:^|\/)\.?openspec\/specs\/([^/]+)\/(.+)$/)
    if (capability) {
      const name = capability[1]
      add(
        `spec:${name}`,
        { title: name, kind: "spec" },
        { label: capability[2], path, isSpec: true },
      )
      continue
    }
    // --- speckit ---
    // A feature's documents (spec/plan/tasks/research/…) under top-level specs/.
    const feature = speckit ? path.match(/(?:^|\/)specs\/([^/]+)\/(.+)$/) : null
    if (feature) {
      const name = feature[1]
      add(
        `change:${name}`,
        { title: name, kind: "change" },
        { label: feature[2], path, isSpec: false },
      )
      continue
    }
    // speckit project memory (constitution, …).
    const memory = path.match(/(?:^|\/)\.specify\/([^/]+)\/(.+)$/)
    if (memory) {
      const name = memory[1]
      add(`spec:${name}`, { title: name, kind: "spec" }, { label: memory[2], path, isSpec: true })
    }
  }
