...
function snapshot(): string {
  ...
  for (const el of Array.from(document.querySelectorAll(SELECTOR))) {
    ...
    const nm = accName(el)
    out.push(
      `${ref} ${role(el)}${nm ? ` "${nm}"` : ""}${flags.length ? ` [${flags.join(",")}]` : ""}`,
    )
    if (n >= 500) {
...
// biome-ignore lint/suspicious/noExplicitAny: the driver sends arbitrary JSON commands, narrowed per branch below
async function handle(cmd: any): Promise<{ ok: boolean; value?: unknown; error?: string }> {
  try {
    switch (cmd.action) {
      ...
      case "press": {
        const target =
          cmd.ref || cmd.selector ? resolve(cmd) : document.activeElement || document.body
      ...
      case "getByText": {
        ...
        const hit = all.find(
          (e) =>
            isVisible(e) &&
            (e.textContent || "").trim().includes(cmd.text) &&
            e.children.length === 0,
        )
