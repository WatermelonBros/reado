...
The companion SHALL be draggable by its character and SHALL be placeable in any
of the four corners of any display — bottom-right, bottom-left, top-right,
...
choice rather than assume one corner. It SHALL remember where it was put per
display, and SHALL return to a default corner when that display is gone.
...
#### Scenario: A display is disconnected

- **WHEN** the companion was parked on an external display that is later disconnected
- **THEN** it reappears in the default corner of the remaining display
...
A click on the character SHALL perform the single action its current state
implies: for a handoff, raise Reado and focus the terminal pane that handed back;
for a failed test run, raise Reado and open the Tests panel at the failure; when
idle, raise Reado. It SHALL NOT open a menu of choices on a plain click; the
...
- **THEN** Reado comes to the front with the pane that handed back focused
