...
**Reason**: Detection was specified per project, from configuration — a project
declaring its framework through `devDependencies`, `Cargo.toml`, `pyproject.toml`
or `go.mod`, with the matching entry selected for the whole project. Two things
are wrong with that and both matter here. A repository with Rust and TypeScript
tests side by side has no single answer, and choosing one makes the other
unrunnable; and reading configuration means a project whose dependencies were
never installed shows no tests at all, which is exactly when reading them from
the source is most useful.

**Migration**: Replaced by *Per-File Framework Recognition* below, which keeps
the registry — one entry per framework, contributing its commands and its output
parser, so adding one stays additive — and moves the choice from the project to
the file.
