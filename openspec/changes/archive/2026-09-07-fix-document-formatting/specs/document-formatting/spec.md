...
Reado SHALL select a CLI formatter for a file only from formatters the project
declares. A formatter is declared when the project root contains a
configuration file that formatter owns, or names it as a dependency in the
project's package manifest. Where more than one formatter could plausibly serve
a file type, Reado SHALL NOT fall back to a binary found only on the global
PATH; where a language has a single conventional formatter that ships with its
toolchain, PATH resolution SHALL remain the declaration.
...
- **WHEN** the project declares no formatter for the file's type
...
- **WHEN** the user formats a Rust or Go file and the toolchain's formatter is
  installed
- **THEN** Reado formats with it, because the language has one conventional
  formatter and no ambiguity to resolve
